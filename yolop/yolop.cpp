...
private:
	...
	const string classesFile = "/home/punzeonlung/CPP/yolop/classnames.txt";
...
void detect_image_with_yolop(Mat& srcimg,YOLO& yolo_model)
{
	int64 start = getTickCount();
	Mat outimg = yolo_model.detect(srcimg);
	float t = (getTickCount() - start) / static_cast<float>(getTickFrequency());
	putText(outimg, format("FPS: %.2f", 1.0 / t), Point(120, 140), FONT_HERSHEY_PLAIN, 2.0, cv::Scalar(255, 255, 255), 2, 8);
...
void detect_video_with_yolop(std::string& video_name,YOLO& yolo_model)
{
	...
	VideoWriter outputVideo("output_video.avi", cv::VideoWriter::fourcc('M', 'P', '4', '2'), fps, cv::Size(frameWidth, frameHeight));
...
int main()
{
	YOLO yolo_model("/home/punzeonlung/CPP/yolop/yolop.onnx", 0.25, 0.45, 0.5);
	//string imgpath = "/home/punzeonlung/CPP/yolop/carlane.jpg";
	//Mat srcimg = imread(imgpath);
	//detect_image_with_yolop(srcimg, yolo_model);
	string video_name = "/home/punzeonlung/CPP/yolop/test2.mp4";
	detect_video_with_yolop( video_name, yolo_model);
