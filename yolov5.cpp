...
void main_detectprocess_with_yolov5(std::string& onnx_path_name, std::vector<std::string>& labels, Mat& frame)
{
	// GPU Mode, 0 - gpu device id
	std::wstring modelPath = std::wstring(onnx_path_name.begin(), onnx_path_name.end());
	Ort::SessionOptions session_options;
	Ort::Env env = Ort::Env(ORT_LOGGING_LEVEL_ERROR, "yolov7-onnx");
	...
	output_h = output_dims[1]; // 25200
	output_w = output_dims[2]; // 85
	...
	// output data 1X25200X85 ,85: CX,CY,W,H, confidence, 80 classes
...
void detect_img_yolov5(std::string& img_name, std::string& onnx_path_name, std::vector<std::string>& labels)
{
	...
	main_detectprocess_with_yolov5(onnx_path_name, labels, frame);
	// FPS render it
...
void detect_video_yolov5(std::string& video_name, std::string& onnx_path_name, std::vector<std::string>& labels)
{
...
int main(int argc, char** argv) {
	...
	std::string onnx_path_name = "D:\\deploy\\models\\yolov5s.onnx";
	std::string video_name = "E:\\opencv\\yolo8Sort\\Videos\\motorbikes.mp4";

	//detect_img_yolov5(img_name, onnx_path_name ,labels);
	detect_video_yolov5(video_name, onnx_path_name, labels);
