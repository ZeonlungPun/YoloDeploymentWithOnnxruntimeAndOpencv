...
auto MainProcessForSingleImage(torch::jit::script::Module yolo_model,torch::Device device,cv::Mat& image,int model_input_height,int model_input_wdith)
{
    cv::Mat input_image;
    letterbox(image, input_image, {model_input_height, model_input_wdith});
    yolo_model.eval();
...
void VisualizeResults(auto keep,cv::Mat& image,std::vector<std::string> classes)
{
  ...
  for (int i = 0; i < keep.size(0); i++) {
      ...
      cv::putText(image, classes[cls], cv::Point(rect.x, rect.y - 10), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 255), 1);
  }
}

void RunForImage(std::string model_path,std::string image_path,std::string save_path,int model_input_height,int model_input_wdith,torch::Device device,std::vector<std::string> classes)
{
  // Load the model (e.g. yolov8s.torchscript)
  torch::jit::script::Module yolo_model = torch::jit::load(model_path);
  // Load image and preprocess
  cv::Mat image = cv::imread(image_path);
  auto keep=MainProcessForSingleImage(yolo_model,device, image, model_input_height,model_input_wdith);
  VisualizeResults(keep, image, classes);
  cv::imwrite(save_path,image);

}

void RunForVideo(std::string model_path,std::string video_path,std::string save_path,int model_input_height,int model_input_wdith,torch::Device device,std::vector<std::string> classes)
{
  // Load the model (e.g. yolov8s.torchscript)
  torch::jit::script::Module yolo_model = torch::jit::load(model_path);
  ...
  cv::VideoWriter video(save_path, cv::VideoWriter::fourcc('M', 'P', '4', 'V'), frame_fps, cv::Size(frame_width, frame_height));
	...
	while (true)
	{
		...
		auto keep=MainProcessForSingleImage(yolo_model,device, frame, model_input_height,model_input_wdith);
    // draw the results and save
    VisualizeResults(keep, frame, classes);
		// FPS render it
...
int main() {
    ...
    try {
        ...
        std::string save_path="/home/kingargroo/YOLOVISION/result.mp4";
        std::string video_path="/home/kingargroo/YOLOVISION/people.mp4";
        int model_input_height=640;
        int model_input_wdith=640;
        //RunForImage(model_path,image_path,save_path,model_input_height,model_input_wdith,device,classes);
        RunForVideo(model_path, video_path, save_path, model_input_height, model_input_wdith, device,classes);
